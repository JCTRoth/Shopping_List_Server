...
Setup:
- Edit appsettings.json
  the database server name and port is specified in appsettings.json. The default is the configuration for docker: "DbServerAddress": "server=db;port=3306;"
  If you are running a local mysql server (on your external server), you can change the path to localhost, e.g. "DbServerAddress": "server=localhost"
...
- Install MySQL
- Create a schema as specified in appsettings.json
```
