...
TIP: Detailed documentation is available in the link:docs/index.adoc[docs/] folder, including the link:docs/api-json-exchange.adoc[API JSON Exchange Reference], link:docs/database-integration.adoc[Database Integration] guide, and link:docs/deployment.adoc[Deployment Guide] (Docker & Kubernetes).
...
[.text-center]
++++
<img src=".doc/launch_screen.jpg" alt="Launch Screen" width="200"/>
<img src=".doc/edit.jpg" alt="Edit Screen" width="200"/>
<img src=".doc/zoom.jpg" alt="Zoom Screen" width="200"/>
++++

Launch Screen | Edit Screen | Zoom Screen
...
* **Entities and Models** (see link:docs/database-integration.adoc#entity-models[Entity Models] for full schema):
...
* **Database Context**: [`AppDb`](Database/AppDb.cs) manages database operations with lazy loading, UTC time conversion, and migration support. See link:docs/database-integration.adoc[Database Integration] for the full schema and hybrid storage architecture.
...
* **Permissions**: Enum-based flags for shopping list access, allowing combinations like WriteAndAddPermission. Permissions are checked at the service layer. See link:docs/api-json-exchange.adoc#permission-types[Permission Types] for the full enum definition.
...
See link:docs/database-integration.adoc[Database Integration] for complete schema documentation and migration history.
...
For complete JSON request/response formats with examples, see link:docs/api-json-exchange.adoc[API JSON Exchange Reference].
...
Connect to `/shoppingserver/update` for real-time updates on shopping list changes. See link:docs/api-json-exchange.adoc#_signalr_real_time_messages[SignalR Real-Time Messages] for all event types and payload formats.
